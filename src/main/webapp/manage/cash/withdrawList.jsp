...
<body>
	...
	<script type="text/javascript">
				...
				function() {
					...
					$('[data-rel=tooltip]').tooltip({
 					...
 					initButton([ {
 						name : "查询",
 						click : function() {
 							searchFrom('./cash/withdrawListJson?');
 						}
 					} ])
				});
		...
		function operate(cellvalue, options, rowObject) {
			...
			if (rowObject.state == 0) {
				html += '<button class="btn btn-xs btn-primary" onclick="applypass(\''
	 				+ rowObject.type + '\', \'' + rowObject.withdraw_id + '\')" style="margin-right:10px;">申请通过</button>';
			} else if (rowObject.state == 1) {
				html += '<button class="btn btn-xs btn-primary" onclick="successpass(\''
	 				+ rowObject.type + '\', \'' + rowObject.withdraw_id + '\')" style="margin-right:10px;">提现成功通过</button>';
			} 
 			
			return html;
		}
		
		function applypass(type, withdraw_id){
			$.ajax({
	    		 url:'./cash/withdrawApplyPass',
	    		 data:{type:type,withdraw_id:withdraw_id},
	    		 success:function(msg){
	    			if(msg==1){
	 					loadHtml("./cash/withdrawList");
	    			}else if(msg==2) {
	    				alert("提现金额超出余额");
	    			}else{
	    				alert("操作失败");
		...
		function successpass(type, withdraw_id){
			$.ajax({
	    		 url:'./cash/withdrawSuccessPass',
	    		 data:{type:type,withdraw_id:withdraw_id},
	    		 success:function(msg){
	    			if(msg==1){
	 					loadHtml("./cash/withdrawList");
